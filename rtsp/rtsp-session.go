...
func NewSession(server *Server, conn net.Conn) *Session {
	...
	debugLogEnable := utils.Conf().Section("rtsp").Key("debug_log_enable").MustInt(1)
	session := &Session{
...
func CheckAuth(authLine string, method string, sessionNonce string) error {
	...
	var hw_live_user models.User
	err := db.MYSQL.Where("Username = ?", username).First(&hw_live_user).Error
	if err != nil {
		return fmt.Errorf("CheckAuth error : user not exists")
	}
	md5UserRealmPwd := fmt.Sprintf("%x", md5.Sum([]byte(fmt.Sprintf("%s:%s:%s", username, realm, hw_live_user.Password))))
...
func (session *Session) handleRequest(req *Request) {
	...
	//获取用户推流地址，和数据库对比，如果不匹配则停止接收流
	//pushurl, err := url.Parse(req.URL)
	
	var hw_live_list models.List
		
		format := "2006-01-02 15:04:05"
		now, _ := time.Parse(format, time.Now().Format(format))
		logger.Printf("[session]: auth url: %s, time： %s", req.URL, now)
		regrule, _ := regexp.Compile(`[^\/]+`)
		mid:= regrule.FindAllString(req.URL, -1)[2]
		logger.Printf("[session]: mid match: %v,",mid)
		finderr := db.MYSQL.Where("Mid = ? ", mid).First(&hw_live_list).Error
		
		if finderr != nil {
		
			logger.Printf("find mid from db err:%v", finderr)
			res.StatusCode = 404
			res.Status = "Mid not found"
			return
			
		}else{
			
			endtime, _ := time.Parse("2006-01-02T15:04:05+08:00", hw_live_list.End_time)
			//logger.Printf("now:%v, endtime:%v, error:%v",now, endtime,parseerr)
		...
		/*
		if err := db.MYSQL.Where("Mid = ? and End_time < ?", mid, now).First(&hw_live_list).Error; err != nil {
		
			logger.Printf("find mid from db err:%v", err)
			res.StatusCode = 404
			res.Status = "Mid not found"
			return
		...
		if session.authorizationEnable {
			...
			if authLine != "" {
				...
				if err == nil {
					...
					logger.Printf("[Authorization]%v", err)
				}
			}
			if authFailed {
				res.StatusCode = 401
				...
				session.nonce = nonce
				res.Header["WWW-Authenticate"] = fmt.Sprintf(`Digest realm="UpareStream", nonce="%s", algorithm="MD5"`, nonce)
				return
			}
		}
	}
