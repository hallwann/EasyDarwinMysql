...
func (h *APIHandler) ModifyPassword(c *gin.Context) {
	...
	db.MYSQL.First(&hw_live_user, sess.Get("uid"))
	if hw_live_user.ID != "" && strings.EqualFold(form.OldPassword, hw_live_user.Password) {
		db.MYSQL.Model(&hw_live_user).Update("password", form.NewPassword)
	} else {
...
func (h *APIHandler) GetServerInfo(c *gin.Context) {
	c.IndentedJSON(http.StatusOK, gin.H{
		...
		"Server":           fmt.Sprintf("%s/%s,%s (Platform/%s;)", "UpareStream", BuildDateTime, BuildVersion, strings.Title(runtime.GOOS)),
		"memData":          memData,
		"cpuData":          cpuData,
...
func (h *APIHandler) DefaultLoginInfo(c *gin.Context) {
	...
	db.MYSQL.First(&hw_live_user, "username = ?", defUser)
	if utils.MD5(defPass) != hw_live_user.Password {
		defPass = ""
	}
	c.JSON(200, gin.H{
		"username": defUser,
