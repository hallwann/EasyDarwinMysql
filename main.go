...
func (p *program) Start(s service.Service) (err error) {
	...
	go func() {
		...
		for {
			var hw_live_streams []models.Stream
			//db.MYSQL.Find(&hw_live_streams)
			if err := db.MYSQL.Find(&hw_live_streams).Error; err != nil {
				log.Printf("find stream err:%v", err)
				return
			}
			for i := len(hw_live_streams) - 1; i > -1; i-- {
				v := hw_live_streams[i]
				if rtsp.GetServer().GetPusher(v.CustomPath) != nil {
					continue
				}
				agent := fmt.Sprintf("UpareStreamServerGo/%s", routers.BuildVersion)
...
func main() {
	...
	svcConfig := &service.Config{
		Name:        sec.Key("name").MustString("UPARE_Service"),
		DisplayName: sec.Key("display_name").MustString("UPARE_Service"),
		Description: sec.Key("description").MustString("UPARE_Service"),
	}
	...
	if len(tail) > 0 {
		...
		if cmd == "install" || cmd == "stop" || cmd == "start" || cmd == "uninstall" {
			figure.NewFigure("UPARE SERVICE", "", false).Print()
	...
	figure.NewFigure("UPARE.COM", "", false).Print()
